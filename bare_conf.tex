...
(representing CUDA~\cite{jsan10} and OpenCL~\cite{opencl08}) has its own
...
statement. Section~\ref{sec:gener-task-graph}
and~\ref{sec:gener-reso-graph} together describe the generation of the
task from the compiler and the setup of the underlying execution
...
then explains our contributions and improvements to this approach. Next,
...
extracting different kinds of parallelism (task and data)
...
K-way graph partitioning and heterogeneous bin packing heuristics. Our SA
approach out performs the former in 54\% of the instances and the latter in 92\%
of the instances.
